...
- **PR-per-fix is STILL the dominant anti-pattern after 3+ documented attempts to fix it**: one session alone ran PRs #108→#144 (36+ single-fix cycles), polled `get_check_runs` 53x (~29K measured tokens, real cost far higher), and ran `sleep 200-260; echo CI-Fenster` 14x — including turns where a `Monitor` was already armed for the same PR. Writing the rule again has not worked; the fix must be mechanical: before calling `create_pull_request`, check the TaskCreate list and require ≥3 `completed` tasks, and once ANY `Monitor` is armed on a PR, delete/ignore the `sleep N; echo` fallback entirely for that PR — re-arm the same Monitor on timeout instead.
- `git fetch origin main --quiet && git checkout -B <branch> origin/main` recurred 13x in one session (once per PR cycle) — purely a symptom of PR-per-fix; skip it when `git log --oneline -1` already matches the last known main SHA.
- `mcp__github__enable_pr_auto_merge` still fails with "Auto-merge is not enabled" — do not retry, go straight to `merge_pull_request`.
- `git push --force-with-lease` after a `non-fast-forward` rejection is expected once PR-per-fix churn starts; don't treat it as a surprise, just do it immediately.

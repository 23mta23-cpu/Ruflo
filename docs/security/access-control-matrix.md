...
| `provider_profiles` | select rows where `available=true and kyc_status='approved'` (public search) | select/update own row (update blocked from touching `stripe_onboarded`, `stripe_account_id`, `kyc_status`, `meister_verified`; **since 0980 also blocked from ADDING an Anlage-A trade to `category_ids` without `meister_verified`**) | none beyond public search | full — only writer of `stripe_onboarded` (via `stripe-webhook`), of `stripe_account_id` (Connect flow, **not yet built**) and of `kyc_status` (via `pruefung`); `meister_verified` is set by the `a_meisterbrief_festhalten` trigger at approval (0980) |
...
| `offers` | none | provider: insert on open/matched jobs requires verified email (0400), fewer than 3 active strikes (0820), matching track (0480) **and, for a job in an Anlage-A trade, `meister_verified` (0980)**; select own offers; customer: select offers on own jobs | none | full |
| `meisterpflicht_gewerke` (0980) | none | **none** — RLS on, no policy (default deny). The list is only read by `auftrag_braucht_meister()` and two trigger functions, all SECURITY DEFINER; clients get the answer, never the table | none | select |
...
| `notifications` (0860) | none | **select own only** (`empfaenger = auth.uid()`). **Keine update-Policy, keine insert-Policy** — bewusst: dürfte der Empfänger die Zeile ändern, könnte er `zugestellt_am` selbst setzen und damit den Nachweis löschen, dass Werkant die Übermittlung noch schuldet. Gelesen-Markieren nur über `benachrichtigung_gelesen(uuid)`, das ausschließlich `gelesen_am` anfasst und auf `auth.uid()` prüft | wie Anbieter | full — Schreiber sind die Trigger `strike_benachrichtigen` und `beschraenkung_benachrichtigen` sowie `benachrichtigung_anlegen` (service_role) |
| `vertrag_start_pins` (0960, 0970) | none | **select nur der Auftraggeber dieses Vertrags**; kein insert, update oder delete | none | full — geschrieben ausschliesslich vom Trigger `start_pin_erzeugen` und von `arbeit_beginnen()`. Die vierstellige Zahl steht im Klartext, weil der Kunde sie vorlesen können muss; geschützt wird sie über die Policy, nicht über die Form. Seit 0970 wird sie beim Einlösen und beim Ende des Vertrags **gelöscht** (Art. 5 Abs. 1 lit. c DSGVO) — der Beleg (`eingeloest_am`, `fehlversuche`) bleibt. Aus der Auskunft nach Art. 15 ist die Zahl selbst ausgenommen und unter `nicht_enthalten` benannt. |
| `job_benachrichtigungen` (0950) | none | none | none | full — RLS enabled, **keine Policy** (default deny). Hält fest, welcher Betrieb über welchen Auftrag informiert wurde; daraus leitet `benachrichtigte_betriebe_nachziehen` die Zahl an `jobs` ab. Geschrieben von `notify-matching-providers` (service_role) und vom Trigger `betrieb_betritt_markt` (SECURITY DEFINER). Wer das lesen könnte, wüsste, welcher Betrieb welchen Auftrag gesehen hat — das geht weder Kunden noch andere Betriebe etwas an. |
...
| `fertigstellung_melden(contract)` (Migration 0770) | `authenticated` only | Caller (`auth.uid()`) muss `contracts.provider_id` sein; Vertrag muss `active` sein und Escrow einbezahlt | Setzt die Abnahmefrist (§ 640 Abs. 2 BGB, 14 Tage) und speichert den Hinweistext nach Satz 2 im **Wortlaut**. Idempotent — ein zweiter Aufruf verlängert eine laufende Frist nicht |
| `abnahme_fiktiv_eingetreten(contract)` (Migration 0770) | jeder (lesend, `stable`) | Kein Aufrufer-Check nötig — liefert nur einen Wahrheitswert über einen Vertrag, den der Aufrufer benennen muss | Einzige Wahrheitsquelle für die Voraussetzungen des § 640 Abs. 2: Fertigstellung gemeldet, Frist gesetzt und abgelaufen, Hinweis belegt, kein offener Mangel, Escrow da, nichts erstattet, keine Rückbuchung |
| `abnahme_faellige_vertraege(limit)` (Migration 0770) | **`service_role` only** — `execute` ist `public`/`anon`/`authenticated` entzogen | Kein Aufrufer-Check nötig, weil Client-Rollen sie nicht ausführen dürfen | Liste der Verträge für den geplanten Lauf |
| `payout_claim(contract, caller, fiktiv)` (Migrationen 0650/0770) | **`service_role` only** — `execute` ist `public`/`anon`/`authenticated` entzogen | Zwei zulässige Wege: (a) `caller` = `contracts.customer_id`; (b) `fiktiv = true` **und** `caller is null` **und** `abnahme_fiktiv_eingetreten` — beides unter `select … for update` auf der Vertragszeile | Ein ausdrücklicher Parameter statt „caller ist null": ein versehentlich durchgereichtes `null` würde sonst still den automatischen Weg öffnen |
...
| `release-escrow` | User JWT **oder** `x-admin-secret` (geplanter Lauf, Konstantzeit-Vergleich) | Mit JWT: Caller muss `contracts.customer_id` sein. Mit Secret: **keine** Eigentumsprüfung in der Function — die Berechtigung ist die abgelaufene Abnahmefrist, und die prüft `payout_claim(…, p_fiktive_abnahme := true)` in der DB (0770) innerhalb derselben Transaktion und Zeilensperre wie die Auszahlung. In beiden Fällen: **Zahlung wird vor jedem Transfer gegen Stripe geprüft** (PaymentIntent `succeeded`, `amount_received` ≥ `customer_total`, `metadata.contract_id` passend) | 10/min per user, 30/min per IP | Idempotency key per contract |
...
| `send-push` | User JWT required | Caller and target must share a job or contract | 20/min per user, 60/min per IP; zusätzlich E-Mail-Takt 1/30 min je Empfänger für Chat (`mailtakt:<user>:chat`) | Prevents using push as a spam vector against strangers. Der E-Mail-Takt schützt den Ruf der Absender-Domain: darunter litten zuerst die Pflichtmitteilungen (AGB §7(4), DSA Art. 17). Push bleibt ungetaktet. |
...
| `zustellung` (0860) | `x-admin-secret` (nicht User-JWT), Konstantzeit-Vergleich | Keine Eigentumsprüfung nötig: die Function wählt die Empfänger nicht selbst, sondern bekommt sie aus `unzugestellte_pflichtmitteilungen()` (service_role, liefert nur Pflichtmitteilungen mit `zugestellt_am is null`). E-Mail-Adressen verlassen die Function nur Richtung Resend | 5/min per IP, **vor** dem Secret-Vergleich | Verschickt Pflichtmitteilungen nach AGB §7(4) / Art. 4 P2B-VO und DSA Art. 17. Quittiert **erst nach** der Annahme durch Resend, und über `zustellung_quittieren()` in Mitteilung **und** Ursprungsvorgang zugleich. Fehlt `RESEND_API_KEY`, antwortet sie 503 statt Erfolg zu melden — `zustellung_stau` in `/health` bleibt dann sichtbar rot |

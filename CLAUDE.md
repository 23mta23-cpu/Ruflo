...
- `app/(tabs)/index.tsx` was Read 4x (~5,687 tokens wasted) across separate task phases in the same session (initial exploration, then again after a TaskCreate-driven bugfix pass) — once a file is read early in a session, keep its content in context across later phases of the SAME session instead of re-reading it when a new task references it again.
- Small-file Edit loops recur across almost every screen touched in a multi-bug pass: `app/(tabs)/index.tsx` 11x, `app/auftrag-detail.tsx` 9x, `app/(tabs)/auftraege.tsx` 4x, `app/einstellungen.tsx` 4x, `app/login.tsx` 4x, `app/auftrag-aufgeben.tsx` 3x, `app/(tabs)/nachrichten.tsx` 3x. When a file needs multiple unrelated fixes in one pass, plan every change from a single Read, then apply via one `python3 - <<EOF` read+replace+write instead of sequential `Edit` calls (already the fallback used successfully for `registrierung.tsx`, `onboarding-kyc.tsx`, `anbieter-warteliste.tsx` in this session).
...
- `Monitor` on a single PR's CI checks reliably **times out before checks finish** (observed timing out on PR #110's check run at msg 678) — the fallback that happened was a manual `get_check_runs` poll loop (6x across PRs #110-#112, ~2,913 tokens) interleaved with ad-hoc `sleep 200-220` filler commands. On a Monitor timeout, re-arm the SAME Monitor call (it supports an until-loop) instead of switching to a sleep+manual-poll pattern — one re-armed Monitor beats N `get_check_runs` calls.

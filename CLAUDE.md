...
- Re-reading the same Playwright verification screenshot (e.g. `scratchpad/home.png`) at full resolution across separate design-iteration rounds cost ~132,000 tokens over 4 reads in one session — each full-res PNG (~170-200KB, ~900-2000px) costs ~30-40K vision tokens per Read.
- Before `Read`-ing a verification screenshot, downscale it first (e.g. `python3 -c "from PIL import Image; Image.open(p).resize((w//2,h//2)).save(p)"` to ~800px width) — layout/color checks don't need full mobile resolution, and this cuts the per-screenshot token cost roughly in half.
- Batch multiple UI fixes into one edit pass before re-exporting + re-screenshotting, rather than taking a fresh full screenshot after every single small change.

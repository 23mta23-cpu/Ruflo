...
- ⬜ ⚖️ **AGB final prüfen** — Fernabsatz/Verbraucherrecht (§312 ff. BGB), **P2B-VO** (EU 2019/1150:
  Ranking-Transparenz + Kündigungsfristen ggü. Handwerkern). #blocker #recht
- ⬜ ⚖️ **Datenschutzerklärung prüfen** — DSGVO Art. 13/14, Rechtsgrundlagen, Speicherfristen. #blocker #recht #dsgvo
- ⬜ ⚖️ **Widerrufsbelehrung** exakt nach gesetzlichem Muster (Anlage EGBGB). #blocker #recht
...
- ⬜ 🤖 **Firmenadresse** ersetzen in `app/impressum.tsx`, `app/agb.tsx`, `app/datenschutz.tsx`, `app/widerruf.tsx` (aktuell `Musterstraße 1, 50667 Köln`). #blocker
- ⬜ 🤖 **Geschäftsführer-Name** ersetzen in `app/impressum.tsx`, `app/datenschutz.tsx` (aktuell `[Ihr Name]`). #blocker
- ⬜ 🤖 **HRB-Nr. + USt-IdNr.** ersetzen (aktuell Platzhalter `HRB 123456` / `DE123456789`). #blocker

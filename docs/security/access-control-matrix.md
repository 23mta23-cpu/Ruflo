...
| `export-my-data` | User JWT required | Alle Queries strikt auf `auth.uid()` gescoped (eigene Profile/Jobs/Offers/Contracts/Reviews; Messages nur aus eigenen Jobs) | 3/hour per user, 6/hour per IP | Art. 20 DSGVO Datenportabilität; JSON-Antwort direkt an den Client (kein Mailversand) |

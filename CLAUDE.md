...
*Auto-generated by `headroom learn` on 2026-07-12 — do not edit manually*
...
- `Read` occasionally fails with `InputValidationError: ... could not be parsed as JSON` when the `offset` argument is malformed mid-call (seen on `app/auftrag-aufgeben.tsx` and `__tests__/categories.test.ts`) — just retry the same Read call with an explicit integer offset/limit; it's a transient serialization glitch, not a real file problem.

### GitHub Pages Deploy
*~1,800 tokens/session saved*
- Despite prior guidance, this session again shipped 15+ separate micro-PRs (one per small wizard/UX/copy fix), each with its own `until h=$(curl ... entry-*.js ...)` poll loop (10 occurrences, ~544 tokens) — when several related fixes land within the same short exchange, batch them into one commit/PR before merging, instead of merging after every single message.
- When a deploy poll must run standalone, prefer a single backgrounded `sleep 400 && curl ...` (Bash `run_in_background`) over a synchronous `until ... $RANDOM ...` loop re-checked on every reply.
- `static.yml` is the sole workflow wired to GitHub Pages (deploy-web.yml was removed 2026-07-02). Deploys take 5-7 min. Confirm via bundle hash: `curl -s "https://23mta23-cpu.github.io/Ruflo/?v=$RANDOM" | grep -o '_expo/static/js/web/entry-[a-f0-9]*\.js'` — never via `mcp__github__actions_list` (350k+ chars, max-token error); use `curl -s "https://api.github.com/repos/23mta23-cpu/Ruflo/actions/runs?per_page=5"` for run status.
...
- Never combine `pkill -f spa-server` (or any pkill targeting a background process) with a subsequent `git commit`/`git add` in the same Bash call — pkill's SIGTERM propagates to the whole command chain and aborts it with exit code 144. Run `pkill -f spa-server 2>/dev/null; true` as its own call, then commit separately.
- Playwright screenshot verification loop for this repo: `npx expo export --platform web`, then start `python3 <scratchpad>/spa-server.py` (serves `dist/` on :8744), confirm with `curl -s -o /dev/null -w "%{http_code}" http://localhost:8744/`, then run the parameterized `<scratchpad>/shot2.cjs <pfad> <name>` (Consent wird via addInitScript vorab gesetzt) — kein neues .cjs pro Check schreiben.
- Chromium binary: `/opt/pw-browsers/chromium_headless_shell-1194/chrome-linux/headless_shell` (einmal per `find /opt/pw-browsers -iname headless_shell` ermittelt — nicht raten). CDN-Skripte (jsdelivr etc.) blockt der Proxy.

...
*Auto-generated by `headroom learn` on 2026-07-18 — do not edit manually*
...
- For GitHub Actions run status: prefer `mcp__github__actions_list` with `method: list_workflow_runs` and a narrow `resource`/filter param — raw curl to the REST API can return an incomplete JSON body (missing `workflow_runs`) through the sandbox proxy. Each call returns 18k-48k tokens — call it AT MOST ONCE per deploy-verification, never re-poll with identical params (one session repeated it 3x for ~18k wasted tokens).
- **PR-per-fix is a confirmed, still-recurring anti-pattern at large scale**: one session ran ~62 separate branch-checkout → commit → PR-create → PR-merge cycles (PRs #38-#99), most for single-file fixes. Default to accumulating 3-5 related fixes on the same branch before opening one PR, unless the user explicitly says "sofort"/"jetzt" or it's a standalone security fix. This has been flagged repeatedly and is NOT yet fixed in practice — treat it as the single highest-priority discipline gap.
- Don't re-run `git fetch origin main && git checkout -B <branch> origin/main` (or the 4-retry `for i in 1 2 3 4; do git fetch ...` variant) before every single edit/commit — one session repeated the checkout variant 15x (~3.3k tokens) and the retry-fetch variant 33x (~1.8k tokens) with no intervening branch switch. Fetch/checkout ONCE per work block; only re-fetch if you know origin/main moved.
- After triggering a workflow run or merging a PR that redeploys GitHub Pages, use ONE `sleep <5-7min> + TaskOutput` wait per deploy batch, not one wait per individual PR merge — one session issued 7 separate `sleep 40/180/240/300/360` waits (~390+ tokens of pure wait-loop overhead) interleaved with single-fix PRs, which is the same anti-pattern as PR-per-fix.
